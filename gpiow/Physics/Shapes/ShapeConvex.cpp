...
Vec3 CalculateCenterOfMass( const std::vector< Vec3 > & pts, const std::vector< tri_t > & tris ) {
	...
	Vec3 cm( 0.0f, 0.0f, 0.0f );
	const float dx = bounds.sizes()[0] / (float)numSamples;
	const float dy = bounds.sizes()[1] / (float)numSamples;
	const float dz = bounds.sizes()[2] / (float)numSamples;
...
Vec3 ShapeConvex::Support( const Vec3 & dir, const Vec3 & pos, const Quat & orient, const float bias ) const {
	...
	Vec3 maxPt = QuatRotatePoint( orient, m_points[ 0 ] ) + pos;
	float maxDist = dir.Dot( maxPt );
	for ( int i = 1; i < m_points.size(); i++ ) {
		const Vec3 pt = QuatRotatePoint( orient, m_points[ i ] ) + pos;
...
Bounds ShapeConvex::GetBounds( const Vec3 & pos, const Quat & orient ) const {
	...
	Bounds bounds;
	for ( int i = 0; i < 8; i++ ) {
		corners[ i ] = QuatRotatePoint( orient, corners[ i ] ) + pos;
