...
Vec3 Body::GetCenterOfMassWorldSpace() const {
	...
	const Vec3 pos = m_position + QuatRotatePoint( m_orientation, centerOfMass );
	return pos;
}
...
void Body::Update( const float dt_sec ) {
	...
	// T = Ia = w x I * w
	// a = I^-1 ( w x I * w )
	Mat3 orientation = QuatToMat3(m_orientation);
	...
	Quat dq = QuatCreate( dAngle, dAngle.GetMagnitude() );
